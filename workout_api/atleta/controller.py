...
async def post(db_session: DatabaseDependecy, atleta_in:AtletaIn=Body(...)):
    ...
    if not centro_treinamento:
         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail =f'centro de treinamento {centro_treinamento_name} não encontrada')

    try:
        atleta_out= AtletaOut(id=uuid4(), created_at=datetime.utcnow(), **atleta_in.model_dump())
        atleta_model= AtletaModel(**atleta_out.model_dump(exclude={'categoria','centro de treinamento'}))
...
async def query(id: UUID4 ,db_session: DatabaseDependecy) ->AtletaOut:
    atleta: AtletaOut = (await db_session.execute(select(AtletaModel))).filter_by(id=id).scalars().firts()
